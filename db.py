...
async def db_find_user(interaction: discord.Interaction, query: str):
    try:
        (cnx, cur) = db_connect()
        cur.execute(f"""
            SELECT
            ...
            WHERE users.ID = %s OR LOWER(email) = LOWER(%s) OR LOWER(characters.`name`) = LOWER(%s) OR LOWER(users.`username`) = LOWER(%s)
